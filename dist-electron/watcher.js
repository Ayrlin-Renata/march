...
let getLabelFn = () => 0;
export function setLabelLookup(fn) {
    getLabelFn = fn;
}
export function setupWatcher(mainWindow, watchPaths, lookbackDays = 3) {
    ...
    const initialBuffer = [];
    let isReady = false;
    const processFile = (filePath, isInitial = false) => {
        try {
...
export function reBroadcastFiles(mainWindow) {
